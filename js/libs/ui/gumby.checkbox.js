...
!function($) {
	...
	Checkbox.prototype.inTable = function(check) {
		
		if(check) {
			
		}
		if( this.$el.parent().is( "td" ) ) {
			if(check) {
				this.$el.parent('td').parent('tr').addClass('active');
			} else {
				this.$el.parent('td').parent('tr').removeClass('active');
			}
		}
	}
	...
	Checkbox.prototype.update = function(check) {
		...
		if(check) {
			...
			if (this.$el.parent().is( "td" )) {
				this.$el.closest('tr').addClass('active');
			}
		...
		// uncheck checkbox - uncheck input, remove checked class, remove <i>
			...
			if (this.$el.parent().is( "td" )) {
				this.$el.closest('tr').removeClass('active');
			}
